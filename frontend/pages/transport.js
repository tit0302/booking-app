...
export default function Transport() {
  ...
  const fetchServices = async () => {
    ...
    setError('');
    try {
      const res = await axios.get(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001'}/api/service?type=transport`);
  ...
  return (
    <div className="min-h-screen bg-secondary-50 py-12 px-4">
      <div className="max-w-6xl mx-auto">
        <h1 className="text-3xl font-bold mb-8 text-primary-700">Dịch vụ Vận chuyển</h1>
        ...
        {loading ? (
          ...
          <div className="text-center text-gray-500 py-12">Không có dịch vụ phù hợp.</div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filtered.map(s => (
              <div key={s._id} className="card card-hover p-6 flex flex-col">
                ...
                <div className="text-primary-700 font-bold mb-2">{s.price?.toLocaleString()} đ/chuyến</div>
                <Link href={`/transport/${s._id}`} className="btn-primary mt-auto text-center">Xem chi tiết</Link>
              </div>
            ))}
          </div>
        )}
